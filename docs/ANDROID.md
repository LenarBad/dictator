...
Плитка — замена хоткея, когда клавиатуру менять не хотят. На **Android 14+** система не даёт поднять `microphone` FGS прямо из `TileService` (while-in-use). Поэтому тап по плитке открывает короткую activity «Говорите… / Стоп»; текст только в буфер + тост.
