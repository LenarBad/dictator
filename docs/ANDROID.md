...
Плитка — замена хоткея. Тап всегда обрабатывается (не `STATE_UNAVAILABLE`: на части OEM клик тогда глотается). Сессия — `TileService.showDialog` («Говорите… / Стоп»): так Android 14+ считает UI видимым и разрешает `microphone` FGS. Текст только в буфер + тост.
...
Foreground service: тип `microphone`, канал «Dictator»: «Запись…» / «Распознавание…». Без уведомления Android 10+ обрежет запись из плитки.
