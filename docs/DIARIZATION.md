...
```
WAV 16 kHz mono (как пишет recorder, до max_recording_seconds)
        │
...
Кластеризация v1: `num_clusters = -1`, `threshold = 0.5` (sherpa default). Число собеседников в настройках — follow-up. `min_duration_on = 0.3`, `min_duration_off = 0.5`.
...
- **RTF.** Диаризация + N вызовов GigaAM длиннее одной транскрипции. Для лимита 180 с это ожидаемо; не стримить по ходу записи в v1.
