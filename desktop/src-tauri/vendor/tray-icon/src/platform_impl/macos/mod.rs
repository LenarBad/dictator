...
use objc2_app_kit::{
    NSCellImagePosition, NSEvent, NSImage, NSMenu, NSSquareStatusItemLength, NSStatusBar,
    NSStatusItem, NSTrackingArea, NSTrackingAreaOptions, NSView, NSWindow,
};
...
impl TrayIcon {
    ...
    pub fn show_menu(&self) {
        if let Some(ns_status_item) = &self.ns_status_item {
            unsafe {
                if let Some(menu) = self.attrs.menu.as_ref().and_then(|menu| {
                    Retained::retain(menu.ns_menu().cast::<NSMenu>())
                }) {
                    if menu.numberOfItems() > 0 {
                        // performClick: no-ops on some Spaces; popUpStatusItemMenu
                        // positions off the status item itself.
                        #[allow(deprecated)]
...
fn on_tray_click(this: &TrayTarget, button: MouseButton) {
    ...
    unsafe {
        ...
        {
            // Clone the ObjC retain and drop the RefCell borrow before the
            // modal popup so a menu action can call set_menu without panicking.
...
fn send_mouse_event(
    ...
    unsafe {
        ...
        // Status-item clicks sometimes have no NSEvent.window. Do not panic:
        // mouseDown already continues to on_tray_click after this call.
        let Some(window) = event.window(mtm).or_else(|| {
            this.ivars()
                .status_item
                .button(mtm)
